...
(defun resultado (estado accao)
	(let ((n 17))
	(let((count 0))
	(desenha-estado(estado accao))
	(cons(car (estado-pecas-por-colocar estado) )(estado-pecas-colocadas estado))
	((estado-pecas-por-colocar estado)(cdr(estado-pecas-por-colocar estado)))
	(cond(estado-final-p (estado))
		estado)
	(cond(and ((estado-final-p (estado))(= nil)))
		(dotimes (i n)
			(cond(tabuleiro-linha-completa-p(estado-tabuleiro estado))
				tabuleiro-remove-linha(estado-tabuleiro estado n)
				(+ 1 count)))
		(cond(eq (count 1))
			(+ 100 estado-pontos estado))
		(cond(eq (count 2))
			(+ 300 estado-pontos estado))
		(cond(eq (count 3))
			(+ 500 estado-pontos estado))
		(cond(eq (count 4))
			(+ 800 estado-pontos estado)))
)))
